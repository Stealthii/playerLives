...
public class MySQL extends DbWrapper {
    ...
    public void open(boolean firstTime) {
        Log.d(firstTime
              ? "Starting MySQL Database Engine..."
              : "Reconnecting to lost MySQL Database server...");

        // Make MySQL connection:
        try {
            new com.mysql.jdbc.Driver();
            con = DriverManager.getConnection("jdbc:mysql://" + parent.conf.dbHost + ":" + parent.conf.dbPort + "/"
                                              + parent.conf.dbDatabase, parent.conf.dbUser, parent.conf.dbPassword);
        ...
        String query = "SELECT table_name FROM information_schema.tables WHERE table_schema = '"
                       + parent.conf.dbDatabase + "' AND table_name = '" + parent.conf.dbTable + "'";
        ...
        try {
            ...
            if (!result.first())    // Table does not exist
            {
                ...
                query = "CREATE TABLE `" + parent.conf.dbTable
                        + "` (`name` VARCHAR(255) NOT NULL,`lives` INT NOT NULL,PRIMARY KEY (`name`))";
    ...
    public boolean set(String player, int lives) {
        ...
        String query = "UPDATE `" + parent.conf.dbTable + "` SET `lives` = '" + lives + "' WHERE `name` = '" + player
                       + "' LIMIT 1";
        boolean ret = false;
...
//      parent.onNumLivesChange(player);  //TODO: Implement
    ...
    public boolean give(String player, int lives)    // MySQL optimized give function
    {
        ...
        String query = "UPDATE `" + parent.conf.dbTable + "` SET `lives` = `lives`+'" + lives + "' WHERE `name` = '"
                       + player + "' LIMIT 1";
        boolean ret = false;
